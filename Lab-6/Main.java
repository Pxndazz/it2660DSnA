class Main 
{
																...
																// I attempting to do MyMap<K, V> hashMap but that wasn't working.
	{
		...
		{
			System.out.println("[" + key.hashCode()  + "\t" + key + "\t\t" + hashMap.get(key) + "]"); //returns negative hashcode and duplicates if using % 10 ? adding * -1 works but i'm not sure if that leads to unintended results
		}
	}
}
